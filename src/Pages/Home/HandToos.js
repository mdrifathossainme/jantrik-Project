...
const HandToos = () => {
  ...
  return (
    <>
      <Swiper
        slidesPerView={
          window.innerWidth < 600 ? 1 : 2 || window.innerWidth > 900 ? 4 : 2
        }
        spaceBetween={30}
        slidesPerGroup={1}
        pagination={{
      ...
      >
        {hproduct.map((product) => (
          <SwiperSlide key={product._id} className="handeItem">
            <div className="p-8 cart-body  delay-500">
              <h6
                className={
                  product.discount &&
                  " absolute lg:left-4 left-2 md:left-0  top-2 rounded-none lg:top-8 btn btn-primary btn-sm text-white"
                }
              >
                {product.discount}
              </h6>
