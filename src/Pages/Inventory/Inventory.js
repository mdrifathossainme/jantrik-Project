...
const Inventory = () => {
  ...
  const {
    ...
    isLoading,
    refetch,
  } = useQuery("singleProduct", () => fetch(url).then((res) => res.json()));
    ...
    <div className="w-[80%] my-12 mx-auto py-12 ">
      <div className="grid md:grid-cols-2 grid-cols-1 md:gap-x-4 lg:gap-x-0 gap-y-4">
        <div className="flex justify-center">
          <img className="w-96 border-2" src={item?.img} alt="" />
        </div>
        <div>
          <h4 className="text-4xl mb-3">{item?.name}</h4>
          <img className="w-[100px]" src={item?.rimg} alt="" />
          <p>{item?.description}</p>
          <h6 className="mt-2 font-semibold">
            IN STOCK: {item?.avilableQuantity}
          </h6>
          <h6 className=" mb-2 font-semibold">
            Minimum order: {item?.orderquantity}
          </h6>
          <h6 className=" mb-2 font-semibold">
            ...
            <form onSubmit={handleSubmit}>
              <div className="qunantity">
                <h6>
                  ...
                  <span>
                    <input
                      className="border-2 ml-2 name  border-gray-300 p-2 w-36 h-8 rounded-md mr-4"
                      onChange={handleQuanty}
                      value={
                        toggleQuntity === false
                          ? item?.orderquantity
                          : newaQuantity.quantity
                      }
                      type="number"
                      id="ide"
                    />
